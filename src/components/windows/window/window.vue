...
<script>
  ...
  export default {
    ...
    created: function () {
      ...
      this.taskInfo = store.getters["info/getTaskInfo"](this.itemId)
      if (this.wndInfo) {
        this.title = this.wndInfo.title;
        this.iconImage = this.wndInfo.icon
        this.windowStyle.left = this.wndInfo.rect.l + "px";
        this.windowStyle.top = this.wndInfo.rect.t + "px";
      ...
      this.source = 'components/layout/laytest.vue';
    ...
    mounted: function () {
      ...
      let top = (this.getScreenHeight - parseInt(this.windowStyle.height, 10)) / 2
      if (top < 50) top = 50

      this.windowStyle.top = top + 'px'
    ...
    methods: {
      ...
      childComp: function () {
        ...
        if (this.source) {
          ...
          return this.compChild.control
          //return this.comp;
          //var myComponent =() => import("@" + this.source );
      ...
      registerComponent1: function (comp) {
        comp.control = import("@/" + comp.name);
        return import("@/" + comp.name);
        // return require("@/" + comp);
      },
      ...
      setFocus: function (focus) {
        this.$store.commit('wins/setFocus', {processId: this.processId, focused: focus})
      },
      toggleMinimizeWindow: function () {
        console.debug(this.minimized)
        //if (this.minimized) {
        //  this.$store.commit('wins/unMinimize', this.processId)
        // } else {
        //this.minimizeWindow()
        this.$store.commit('wins/minimize', this.processId)
        //}
      },
      ...
      getTaskBarIconPosition: function () {
        ...
        //let tasks = document.getElementById('taskbaritem')
        let id = 'taskbaritem' + this.processId
        let task = document.getElementById(id)
        if (task != null) {
          let v = task.getBoundingClientRect()
          let ret =
            {
              x: v.left + (v.width / 2),
              y: v.top + (v.height / 2)
    ...
    computed: {
      ...
      getScreenWidth() {
        return this.$store.getters["wins/getScreenWidth"]
      },
      getScreenHeight() {
    ...
    watch: {
      zIndex: function () {
        console.debug("watch zIndex:" + this.zIndex);
        this.windowStyle.zIndex = this.zIndex

      },
      minimized: function () {
        console.debug("watch minimized:" + this.minimized);
